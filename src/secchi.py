...
def write_fixed_secchi_to_file(file, value, start, end, cols, rows, origin=datetime(2008, 3, 1, tzinfo=pytz.utc)):
...
def write_monthly_secchi_to_file(file, values, start, end, cols, rows, origin=datetime(2008, 3, 1, tzinfo=pytz.utc)):
    grid = np.ones((rows, cols))
    ...
    with open(file, "a") as f:
        ...
        for i in range(1, days + 1):
            ...
            f.write(time_str)
            f.write("\n")
            grid[:] = values[(start + timedelta(days=i)).month - 1]
            np.savetxt(f, grid, fmt='%.2f')
